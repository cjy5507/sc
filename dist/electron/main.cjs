...
async function handleStore(store) {
    ...
    try {
        ...
        checkStopped();
        if (mainWindow)
            mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'typing', message: '메시지 입력중' });
        try {
            ...
            if (mainWindow)
                mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'error', message: '메시지 입력 실패' });
        ...
        checkStopped();
        if (mainWindow)
            mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'submitting', message: '메시지 보내기중' });
        let popup = null;
        try {
            ...
            if (mainWindow)
                mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'error', message: '메시지 보내기 실패' });
        ...
        if (popup) {
            ...
            try {
                ...
                checkStopped();
                if (mainWindow)
                    mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'pass-done', message: 'PASS 인증 완료' });
                ...
                if (mainWindow)
                    mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'error', message: 'PASS 인증 대기 3분 초과, 자동화 실패' });
            ...
            if (mainWindow)
                mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'pass-missing', message: 'PASS 인증 팝업 감지 실패' });
            console.log(`[${store.name}] PASS 인증 팝업 감지 실패(팝업이 안 떴거나 이미 닫힘)`);
        }
        // 6. 자동화 완료
        if (mainWindow)
            mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'done', message: '자동화 완료' });
    }
    catch (e) {
