...
exports.waitUntilMidnight = waitUntilMidnight;
exports.humanDelay = humanDelay;
exports.isLastDayOfMonth = isLastDayOfMonth;
exports.isNearMidnight = isNearMidnight;
exports.getLastDayOfMonth = getLastDayOfMonth;
exports.getTimeToMonthEndMidnight = getTimeToMonthEndMidnight;
exports.getTimeUntilMonthEndMidnight = getTimeUntilMonthEndMidnight;
exports.waitUntilMonthEndPreMidnight = waitUntilMonthEndPreMidnight;
exports.executeExactMidnightReservation = executeExactMidnightReservation;
exports.executeReservationWithCookieHandling = executeReservationWithCookieHandling;
/**
 * 자정까지 대기하는 함수
 */
async function waitUntilMidnight(config) {
    const now = new Date();
 ...
 */
async function humanDelay(minMs = 500, maxMs = 1000) {
 ...
 * @returns boolean
 */
function isNearMidnight(thresholdSeconds = 5) {
    const now = new Date();
 ...
 */
function getTimeToMonthEndMidnight() {
    const now = new Date();
 ...
 * @returns number 말일 자정까지 남은 밀리초
 */
function getTimeUntilMonthEndMidnight() {
 ...
 * @returns Promise<void>
 */
async function waitUntilMonthEndPreMidnight() {
    const now = new Date();
    const lastDay = getLastDayOfMonth();
    // 오늘이 말일이 아니면 함수 종료
 ...
 */
async function executeExactMidnightReservation(page, selector) {
    try {
        // 선택자를 배열로 변환
        const selectors = Array.isArray(selector) ? selector : [selector];
        console.log(`자정 예약 실행 준비: ${selectors.length}개의 선택자 사용`);
        // HTML 덤프 저장 함수 (디버깅용)
        const saveHtmlDump = async (prefix) => {
            try {
        ...
        for (const currentSelector of allSelectors) {
            try {
                // 선택자가 유효한지 확인
                if (!currentSelector || typeof currentSelector !== 'string')
                    continue;
                    ...
                    if (isVisible) {
                        ...
                        for (let i = 0; i < 3; i++) {
                            try {
                                ...
                                console.log(`일반 클릭 시도 ${i + 1} 실패: ${clickError.message}`);
                                // 일반 클릭 실패 시 다른 방법 시도
                                try {
                                    ...
                                    console.log(`대체 클릭 방법 실패: ${alternativeClickError.message}`);
                                }
                                if (i < 2)
                                    await new Promise(r => setTimeout(r, 300)); // 300ms 기다린 후 재시도
                            }
                        }
                        if (clickSuccess)
                            break; // 클릭 성공시 루프 종료
                        ...
                        try {
                            ...
                            console.log(`요소 스크롤 시도 실패: ${scrollError.message}`);
                        }
                    }
                }
                else {
                ...
                console.log(`선택자 ${currentSelector} 처리 중 오류: ${selectorError.message}`);
            }
        }
        ...
        try {
            ...
            console.log(`최종 스크린샷 캡처 실패: ${screenshotError.message}`);
        }
        // 로그 기록
        if (clickSuccess) {
            ...
            try {
                ...
                const successContent = await page.evaluate(() => {
                    // 가능한 성공 메시지나 예약 폼 검색
                    const successElements = document.querySelectorAll('.success, .confirmation, form[id*="appointment"], form[class*="appointment"], ' +
 ...
 */
async function executeReservationWithCookieHandling(page, cookieSelector, reservationSelectors) {
    try {
        ...
        try {
            ...
            console.log(`[TimeUtils] 스크린샷 저장 실패: ${e.message}`);
        }
        // 쿠키 동의 팝업 처리
        try {
            ...
            if (cookieElement) {
                ...
                try {
                    ...
                    console.log(`[TimeUtils] 쿠키 버튼 클릭 실패: ${clickError.message}`);
                    // JavaScript 클릭 시도
                    try {
                        await page.evaluate((selector) => {
                            const element = document.querySelector(selector);
                        ...
                        console.log(`[TimeUtils] 쿠키 버튼 JavaScript 클릭 실패: ${jsError.message}`);
                    }
                }
            }
            else {
            ...
            console.log(`[TimeUtils] 쿠키 동의 팝업 처리 중 오류: ${cookieError.message}`);
        }
        // 테스트 모드 확인
        const isTestMode = process.env.NODE_ENV === 'development';
        ...
        if (isTestMode || !isNearMidnight()) {
            try {
                ...
                try {
                    ...
                    console.log(`HTML 구조 분석 실패: ${htmlError.message}`);
                }
                // 컬렉션 선택 요소에 대한 선택자들
                    ...
                    if (collectionElement) {
                        ...
                        if (isVisible) {
                            ...
                            try {
                                ...
                                console.log(`[TimeUtils] 스크린샷 저장 실패: ${screenshotError.message}`);
                            }
                            // 1. 일반 클릭 시도
                            try {
                                ...
                                console.log(`[TimeUtils] 컬렉션 일반 클릭 실패: ${clickError.message}`);
                                ...
                                try {
                                    ...
                                    console.log(`[TimeUtils] 컬렉션 JS 함수 호출 실패: ${jsError.message}`);
                                    // 3. JS click() 시도
                                    try {
                                        await page.evaluate((selector) => {
                                            const element = document.querySelector(selector);
                                        ...
                                        console.log(`[TimeUtils] 컬렉션 JS click() 실패: ${jsClickError.message}`);
                                    }
                                }
                            }
                            if (collectionClicked)
                    ...
                    try {
                        const htmlStructure = await page.evaluate(() => {
                            return {
                                ...
                                agreeButtons: Array.from(document.querySelectorAll('button')).filter(b => b.textContent?.includes('동의') ||
                                    b.getAttribute('name') === 'submit_appointment').map(b => ({
                                    text: b.textContent?.trim(),
                                    name: b.getAttribute('name'),
                                    type: b.getAttribute('type')
                                }))
                    ...
                    try {
                        ...
                        for (const agreeSelector of agreeButtonSelectors) {
                            ...
                            try {
                                ...
                                if (buttonVisible) {
                                    ...
                                    if (isTestMode) {
                                        try {
                                            ...
                                            console.log(`테스트 모드: 동의 버튼 클릭 실패: ${agreeClickError.message}`);
                                            // JS 클릭 시도
                                            try {
                                                await page.evaluate((selector) => {
                                                    const element = document.querySelector(selector);
                                                ...
                                                console.log(`테스트 모드: 동의 버튼 JS 클릭 실패: ${jsClickError.message}`);
                                            }
                                        }
                                    }
                                ...
                                console.log(`동의 버튼 대기 중 오류: ${waitError.message}`);
                            }
                        }
                    }
                    catch (agreeButtonError) {
                        console.log(`동의 버튼 처리 중 오류: ${agreeButtonError.message}`);
                    }
                }
                else {
                ...
                console.log(`컬렉션 선택 프로세스 중 오류: ${collectionProcessError.message}`);
            }
        }
