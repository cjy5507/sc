...
async function moveMouseNaturally(page, selector) {
    try {
        ...
        for (let i = 0; i <= points; i++) {
            ...
            await (0, timeUtils_1.humanDelay)(50, 150); // 이동 중 짧은 지연
        }
        return true;
    }
    catch (err) {
 ...
 * 인간과 같은 클릭을 시뮬레이션
 */
async function humanClick(page, selector, options = { timeout: 5000 }) {
    try {
        // 요소가 존재하는지 확인
        ...
        if (!moveSuccess)
            return false;
        // 클릭 전 짧은 지연 추가
        await (0, timeUtils_1.humanDelay)(200, 500);
        // 클릭
        await page.click(selector, { delay: Math.floor(Math.random() * 50) + 50 });
        // 클릭 후 짧은 지연
        await (0, timeUtils_1.humanDelay)(300, 700);
        return true;
    }
    catch (err) {
