...
export interface ChronodigmOptions {
  onStatusUpdate?: (status: IStoreStatus) => void;
}

export async function runChronodigmScenario(options?: ChronodigmOptions): Promise<'success' | 'fail' | 'timeout'> {
  ...
  try {
    ...
    options?.onStatusUpdate?.({ name: 'chronodigm', status: 'fail', message: '오류 발생: ' + (e as Error).message });
    await browser.close();
    return 'fail';
  }
}
