...
async function handleStore(store: any) {
  ...
  try {
    ...
    // 3. 메시지 입력 및 PASS 인증
    checkStopped();
    if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'message', message: '메시지 입력중' });
    let popup: any = null;
    try {
      // 메시지 입력 대기 및 입력
      await page.waitForLoadState('networkidle');
      ...
      [popup] = await Promise.all([
        new Promise((resolve) => {
          const popupHandler = async (newPage: any) => {
            try {
      ...
      const carrierSelector = carrierSelectors[config.carrier as keyof typeof carrierSelectors];
      if (!carrierSelector) {
      ...
      // 4. 이메일 입력 및 동의
      checkStopped();
      if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'email', message: '이메일 입력중' });
      
      // 예약 페이지를 위한 변수 미리 선언
      let appointmentPage: any = null;
      let appointmentUrl = '';
      ...
      try {
        ...
        // 5. 새 탭에서 예약 페이지 열기
        checkStopped();
        if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'appointment', message: '예약 페이지로 이동중' });
        ...
        try {
          // 직접 URL로 이동하는 함수 정의
          const navigateToAppointmentPage = async (retries = 3): Promise<boolean> => {
              ...
              try {
                ...
                if (currentPage) {
                  try { 
                    let isClosed = true;
                    try {
                      isClosed = await currentPage.isClosed();
                    } catch (e) {
                ...
                try {
                  // URL 확인 - catch 메서드 사용 대신 try-catch 사용
                  let currentUrl = '';
                  try {
          ...
          if (navigationSuccess) {
            if (mainWindow) mainWindow.webContents.send('automation-status', { 
              storeId: store.id, 
              status: 'success', 
      ...
      // 6. 자정까지 대기
      checkStopped();
      if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'waiting', message: '자정 대기중' });
      try {
      ...
      // 7. 예약 버튼 클릭
      checkStopped();
      if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'reserving', message: '예약 진행중' });
      ...
      try {
        ...
        // 인간 같은 마우스 움직임 시뮬레이션
        const moveMouseNaturally = async (selector: string) => {
          try {
            // 요소 위치 가져오기
        ...
        // 인간 같은 클릭 구현
        const humanClick = async (selector: string, options = { timeout: 5000 }) => {
          try {
            // 요소가 보이는지 확인
            ...
            // 페이지 스크롤 조정 (요소가 보이도록)
            await appointmentPage.evaluate((sel: string) => {
        ...
        // 예약 버튼 찾기 함수 정의
        const findAndClickReservationButton = async (retries = 3): Promise<boolean> => {
          for (let i = 0; i < retries; i++) {
            try {
              ...
              try {
                ...
                for (const button of buttons) {
                  ...
                  if (text && (text.includes('시간 선택') || text.includes('예약'))) {
                    // 요소에 임시 ID 할당
                    const tempId = await appointmentPage.evaluate((el: HTMLElement) => {
                      if (!el.id) {
      ...
      if (mainWindow) mainWindow.webContents.send('automation-status', { storeId: store.id, status: 'error', message: `오류 발생: ${e instanceof Error ? e.message : '알 수 없는 오류'}` });
