...
import { Store, AutomationProcess } from '../types';
import { humanClick, moveMouseNaturally } from '../utils/botDetectionAvoidance';
import { humanDelay, getLastDayOfMonth, executeExactMidnightReservation, getTimeUntilMonthEndMidnight } from '../utils/timeUtils';

/**
 * 말일 자정 예약을 위한 서비스
 */
export class MidnightReservationService {
  private mainWindow: BrowserWindow | null;
  private automationProcesses: Record<string, any>;
  private browser: Browser | null = null;
  private context: BrowserContext | null = null;
  private page: Page | null = null;
  private isRunning: boolean = false;
  private scheduledTask: NodeJS.Timeout | null = null;
  private currentStore: Store | null = null; // storeId를 currentStore로 변경
  private statusCallback: ((status: string, message: string) => void) | null = null;

  constructor(
    mainWindow: BrowserWindow | null, 
    automationProcesses: Record<string, any>
  ) {
   ...
   */
  public async startMidnightReservation(url: string, credentials: any): Promise<void> {
    ...
    try {
      ...
      this.updateStatus(
        store.id,
        'waiting',
        `다음 말일(${nextMonthLastDay.toLocaleDateString()})까지 ${daysUntilLastDay}일 남았습니다.`
      );
   ...
   * @returns Promise<void>
   */
  public async stopMidnightReservation(): Promise<void> {
   ...
   */
  public setStatusCallback(callback: (status: string, message: string) => void): void {
   ...
   */
  private updateStatus(storeId: string, status: string, message: string): void {
   ...
   * @returns Promise<void>
   */
  public async scheduleMonthEndMidnightReservation(store: Store): Promise<void> {
    ...
    this.updateStatus(
      this.currentStore.id, // store.id 대신 this.currentStore.id 사용
      'waiting',
      `다음 말일(${nextMonthLastDay.toLocaleDateString()})까지 ${daysUntilLastDay}일 남았습니다.`
    );
   ...
   * @returns Promise<void>
   */
  public async forceClearResources(storeIdToClear?: string): Promise<void> {
          ...
          await Promise.all(pages.map(async (page) => {
            if (!page.isClosed()) {
              await page.close().catch(err => 
                console.error(`${logPrefix} 추가 페이지 닫기 중 오류:`, err)
              );
            }
          }));
          ...
          if (this.browser.isConnected()) {
            ...
            await Promise.all(contexts.map(async (context) => {
              await context.close().catch(err => 
                console.error(`${logPrefix} 추가 컨텍스트 닫기 중 오류:`, err)
              );
            }));
      ...
      if (storeIdToClear && this.automationProcesses) {
        if (this.automationProcesses[storeIdToClear]) {
          ...
          if (process && typeof process.stop === 'function') {
            await process.stop().catch((err: Error) => 
              console.error(`${logPrefix} 프로세스 중지 중 오류:`, err)
            );
          }
